    ...
    if (isset($_POST['modifyProposition'])) {
        if (!empty($_POST['sports'])) {
            if (preg_match($regexLetterAndNumber, $_POST['sports'])) {
                $user->sportName = htmlspecialchars($_POST['sports']);
            } else {
        ...
        if (!empty($_POST['zipCode'])) {
            if (preg_match($regexzipCode, $_POST['zipCode'])) {
                $user->zipCode = htmlspecialchars($_POST['zipCode']);
            } else {
        ...
        if (!empty($_POST['date'])) {
            $user->date = htmlspecialchars($_POST['date']);
        } else {
        ...
        if (!empty($_POST['hour'])) {
            if (preg_match($regexHour, $_POST['hour'])) {
                $user->hour = htmlspecialchars($_POST['hour']);
            } else {
        ...
        if (!empty($_POST['propositionName'])) {
            ...
            $user->propositionName = htmlspecialchars($_POST['propositionName']);
        } else {
...
$showUserProposition = $user->showProposition();
}